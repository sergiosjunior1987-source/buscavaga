...
import { Search, Download, Globe, MapPin, Building2, Loader2 } from "lucide-react";
...
const simulateSearchResults = (cargo: string, regiao: string, extras: string): SearchResult[] => {
  const empresas = [
  ...
  for (let i = 0; i < count; i++) {
    ...
    results.push({
      titulo: `${cargo}${extras ? ` - ${extras}` : ""} ${i % 3 === 0 ? "(Sênior)" : i % 3 === 1 ? "(Pleno)" : "(Júnior)"}`,
      empresa,
      local: regiao || "Brasil",
...
const JobSearchTab = () => {
  ...
  const handleSearch = () => {
    ...
    setSearching(true);
    setTimeout(() => {
      const data = simulateSearchResults(cargo.trim(), regiao.trim(), extras.trim());
      setResults(data);
      setSearching(false);
  ...
  const exportPDF = () => {
    ...
    autoTable(doc, {
      startY: 35,
      head: [["Título", "Empresa", "Local", "Fonte"]],
      body: results.map((r) => [r.titulo, r.empresa, r.local, r.fonte]),
      styles: { fontSize: 8 },
    ...
    <div className="animate-fade-in">
      <div className="mb-6">
        ...
        <p className="text-muted-foreground mt-1">
          Pesquise vagas por cargo e região. Exporte os resultados em Excel, JSON ou PDF.
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            <Input
              ...
              placeholder='Ex: Recursos Humanos, Comprador'
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-foreground mb-1.5">
          ...
          <div>
            ...
            <Input
              ...
              placeholder="Ex: CLT, remoto, sênior"
            />
          </div>
        </div>
